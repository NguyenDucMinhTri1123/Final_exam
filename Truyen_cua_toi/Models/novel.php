...
class novel{
    ...
    static function get_list(){
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $ls[] = new novel($row['id'], $row['name'], $row['img_url'],$row['author_id'],$row['author_name'], 
                $row['type'],$row['status'],$row['detail'],$row['rank_score'],$row['chapter_number'],$row['tag_list_id']);
            }
        }
        $conn->close();
        return $ls;
