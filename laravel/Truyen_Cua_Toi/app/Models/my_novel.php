...
class my_novel extends Model
{
    ...
    static function update_my_novel($novel)
    {
        $conn = db::connect();
    ...
    static function delete_my_novel($novel)
    {
        $conn = DB::connect();
    ...
    static function get_list()
    {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $ls[] = new my_novel($row['id'], $row['novel_id'], $row['author_id'], $row['types']);
            }
        }
        $conn->close();
        return $ls;
